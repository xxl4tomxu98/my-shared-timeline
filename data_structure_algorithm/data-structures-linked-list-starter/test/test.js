    ...
    describe('LinkedList Methods', () => {
        ...
        describe('insert', () => {
            ...
            it('Should return true if node is successfully inserted at index', () => {
                ...
                //inserting here
                linkedList.addToTail('D');
                expect(linkedList.get(1)).to.eql({ value: 'B', next: { value: 'D', next: null } }); // node@(index - 1) points to node@(index)
                expect(linkedList.insert(2, 'C')).to.equal(true); // node@(index - 1) >> new Node >> node@(index)
