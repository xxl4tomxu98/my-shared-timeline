...
describe('The Course model', () => {
  ...
  context("Queries", function () {
    it('can eagerly fetch a Course and associated People data with a course ID', async () => {
      const { lookupCourseAndPeople } = require("../queries/courseLookup.js");

      const record = await lookupCourseAndPeople(5);
      expect(record.name).to.equal('Appreciation');
      expect(record.People.length).to.equal(2);
    });

    it('can return a list of courses by department name', async () => {
      const { lookupCourseByDepartmentName } = require("../queries/courseLookup.js");

      const records = await lookupCourseByDepartmentName('Mathematics');
      expect(records.length).to.equal(3)
      expect(records[0].name).to.equal('Calculus');
      expect(records[0].People).to.equal(undefined); // not eagerly loaded
    });

    it("can lookup a list of Courses higher than a certain level", async () => {
      const { lookupCoursesByLevel } = require("../queries/courseLookup.js");

      const records = await lookupCoursesByLevel(600);
