...
// BONUS: HANDLING MULTIPLE REQUESTS


// let fetches = []

// // for each url we've specified, fetch with init options
// urls.forEach(url => {
//   fetches.push(fetch(url, init))
// })
...
//       // if we've provided dump header (filename we want to create)
//       // if (dump && dump[i]) {
//       if (dump) {
//         let fileName = dump;
//         let statLine = `HTTP/1.1 ${res.status} ${res.statusText}\n`
...
//           if (err) {
//             console.log(`curl (failed writing header) ${err}`)
//           }
//         })
//       }
//     }
...
//         let resTxt = responses.shift()
//         let fileName = output[i];
//         fs.writeFile(fileName, resTxt, (err) => {
//           if (err) {
//             console.log(`curl output error: ${err}`)
//           }
//         })
//       }
//     }
